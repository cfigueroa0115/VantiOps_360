...
export async function GET(
  ...
  try {
    ...
    const data = await getChartData(sql, chartType);
...
async function getChartData(sql: ReturnType<typeof neon>, chartType: string) {
  switch (chartType) {
    case "pareto":
      return sql(`
        WITH counts AS (
          SELECT causa, COUNT(*)::int AS count FROM pqr_records
          WHERE causa IS NOT NULL GROUP BY causa HAVING COUNT(*) >= 5 ORDER BY count DESC
        ), total AS (SELECT SUM(count) AS total FROM counts)
        SELECT causa, count, ROUND(count * 100.0 / total.total, 2) AS percentage,
    ...
    case "cancellation_donut":
      return sql(`
        WITH ranked AS (SELECT causa, COUNT(*)::int AS count FROM pqr_records WHERE causa IS NOT NULL GROUP BY causa ORDER BY count DESC),
        total AS (SELECT SUM(count) AS total FROM ranked), top AS (SELECT causa, count FROM ranked LIMIT 1)
        SELECT top.causa AS category, top.count, ROUND(top.count * 100.0 / total.total, 2) AS percentage FROM top, total
        UNION ALL SELECT 'Otras causas', (total.total - top.count)::int, ROUND((total.total - top.count) * 100.0 / total.total, 2) FROM top, total
      `);
