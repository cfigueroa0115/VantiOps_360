...
describe("EvidenciaPage", () => {
  ...
  it("displays validation data when JSON returns success", async () => {
    ...
    await waitFor(() => {
      expect(screen.getByTestId("status-snapshot-ci-publicado")).toHaveTextContent("abc1234");
    });
  });
  ...
  it("shows unavailable when validation JSON has null fields", async () => {
    ...
    await waitFor(() => {
      expect(screen.getByTestId("status-snapshot-ci-publicado")).toHaveTextContent("No disponible");
